...
def get_semantic_taxonomy(cluster_object: str = "centroids", sample: int = None) -> pd.DataFrame:
    """Downloads taxonomy from S3 and returns them as a pandas dataframe.
    ...
    if sample:
        return download_obj(
            BUCKET_NAME,
            f"outputs/semantic_taxonomy/assignments/semantic_{cluster_object}_{str(sample)}_clusters.parquet",
            download_as="dataframe",
        )
