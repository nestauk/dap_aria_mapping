...
import pandas as pd
from dap_aria_mapping.getters.cooccurrence_network import get_metacluster_network
...
def generate_hierarchy(
    ...
    while n_iter < total_iters and len(node_list) > min_group_size:
        ...
        communities = louvain_communities(
            subgraph, resolution=resolution, seed=seed, weight="weight"
        )
...
if __name__ == "__main__":
    ...
    network = get_metacluster_network()
    ...
    initial_partitions = louvain_communities(
        network, resolution=starting_resolution, seed=config["seed"], weight="weight"
    )
    ...
    if args.local:
        ...
        if args.test:
            ...
            upload_obj(
                output,
                BUCKET_NAME,
                "outputs/metacluster_assignments.parquet",
                kwargs_writing={"index": True},
            )
